...
<TypeTable
  type={{
    "--project <slug>": {
      description:
        "Project slug. Overrides docs.json `slug`. Can also be set with `BLODEMD_PROJECT`. If `slug` is missing, the CLI temporarily falls back to docs.json `name` for backward compatibility.",
      type: "string",
    },
    "--api-key <token>": {
      description:
