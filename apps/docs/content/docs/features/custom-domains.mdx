...
When you run `blodemd push`, your docs are published to `{name}.blode.md`, where `name` is the value in your `docs.json`. No per-project DNS setup is required because the platform wildcard handles `*.blode.md`.
