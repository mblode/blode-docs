...
description: Move a Mintlify docs repo to Blode.md. Copy the MDX, rewrite docs.json to the smaller schema, validate, preview, push, and keep your domain.
---
...
- The CLI: `npm i -g blodemd`
- A Blode.md account: run `blodemd login` once (it opens GitHub OAuth)
...
<Steps>
  ...
  <Step title="Write docs.json">
    Start from the two required keys and add only what Blode.md supports:
    ...
    Map your navigation groups across one at a time. Blode.md `navigation`
    ...
    edit instead of a blank file, `blodemd new` writes a minimal `docs.json`
  ...
  <Step title="Push">
    ...
    install the Blode.md GitHub App from the project's **Git** tab in the
  ...
  <Step title="Keep your domain">
    ...
    - **Docs under a path** (for example `example.com/docs`): keep DNS as is
      and rewrite `/docs` and `/_docs` to your Blode.md site from the server
...
- **The web editor.** Blode.md has no browser editor that commits back. Writing
...
- **Marketplace integrations.** There is no integrations catalog. Analytics is
  a single PostHog key set in the dashboard (or `blodemd analytics set`); see
...
- **Components outside Blode.md's set.** The supported components are
...
- **`blodemd validate` lists keys you never wrote**: nested Mintlify-only
